...
module Sinatra
  module FormHelpers
    ...
    def checkbox(obj, field, options={})
      default_options = { :type => "checkbox", :id => "#{obj}_#{field}", :name => "#{obj}[#{field}]" }
      value = options[:value]
      default_options.merge!({:checked => true}) if @params[obj] && @params[obj][field.to_s] == value
      single_tag :input, options.merge(default_options)
    end
